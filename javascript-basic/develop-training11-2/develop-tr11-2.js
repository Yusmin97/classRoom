function validateId(id) {
  // 타입 검사
  if (typeof id !== "string") {
    return false; // 유효성 검사 측면에서는 error를 리턴하는 것보다 false를 리턴하는
    // 것이 좋은 선택이 될 수 있다.
  }

  let state = { // 두개의 boolean 값을 변수로 관리할 수 있겠지만, 객체로 관리하는
    // 것이 더 편리하다. 관용적으로 이것을 '상태' 라는 뜻의 state라고 한다.
    hasLowerCase: false,
    hasUpperCase: false,
  };
...
;
console.log(validateId("yuSeungmin")); // true
