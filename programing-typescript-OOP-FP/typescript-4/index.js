 ...
 * 실제적인 코드로서는 위 interface만 확인하면 어떤 객체든 무슨 내용이 어떤 구조로 되어있는지 추론 할 수
 * 있게됩니다.
 */
var users = [
    { id: 1, name: "공욱재" },
 ...
 */
function jsonCovertObject(obj) {
    return JSON.parse(JSON.stringify(obj));
}
// 얕은 복사
/**
 * 얕은 복사의 개념은 배열의 원소가 객체일 경우, 원소의 객체를 복사하지 않고 참조만 복사하는 것을 의미합니다.
 * 아래의 전개 연산자(spread operator)를 통해 객체를 복사하고 있지만, 객체의 속성이 객체일 경우, 참조만
 * 복사하게 됩니다.
 */
 ...
 */
var deepCopiedUsers = users.map(function (user) { return jsonCovertObject(user); });
// 복사된 배열에서 객체 수정
 ...
 * 메모리 관점에서 얕은 복사는 원본 객체와 복사된 객체가 같은 객체를 참조하고 있기 때문에, 복사된 객체를
 * 수정하면 원본 객체도 수정되게 됩니다.
 * 깊은 복사는 원본 객체와 복사된 객체가 다른 객체를 참조하고 있기 때문에, 복사된 객체를 수정해도 원복
 * 객체는 수정되지 않습니다.
 *
